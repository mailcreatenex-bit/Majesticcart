...
const CATEGORIES = ['Makeup', 'Skin Care', 'Body Care', 'Hair Care', 'Fragrance'] as const;

// sku, name, category, mrp₹, price₹, BV, gst%, stock, description
const PRODUCTS: [string, string, (typeof CATEGORIES)[number], number, number, number, number, number, string][] = [
...
async function bootstrap() {
  ...
  const plan = existingPlan
    ...
    : await prisma.planVersion.create({
        data: { config: CLIENT_DEFAULT_PLAN as unknown as Prisma.InputJsonValue, note: "Client's plan document, September 2026" },
      });
  ...
  const settings: [string, Prisma.InputJsonValue][] = [
    ...
    ['payment', {
      // Field names here must match what RechargeService.payInfo() reads
    ...
    ['security', { largeRechargePaise: rupeesToPaise(25000).toString(), maxRechargesPerDay: 3, maxAccountsPerDevice: 2 }],
  ];
  ...
  for (const [sku, name, category, mrp, price, bv, gst, stock, description] of PRODUCTS) {
    await prisma.product.upsert({
      ...
      create: {
        ...
        mrpPaise: rupeesToPaise(mrp), pricePaise: rupeesToPaise(price),
        bvCenti: bvToCenti(bv), gstBp: gst * 100, stock, isActive: true,
        hsnCode: '3304',
      },
  ...
  return { plan, company, admin, categoryIds, adminEmail, adminPassword };
}
...
async function seedDemo(ctx: Awaited<ReturnType<typeof bootstrap>>) {
  ...
  const plan = CLIENT_DEFAULT_PLAN;
  ...
  const members: { id: string; code: string; ancestorPath: string; depth: number; sponsorId: string | null }[] = [];
  ...
  const wallet = new Map<string, { shopping: bigint; income: bigint }>();
  const volume = new Map<string, { self: number; group: number; rank: number }>();
  members.forEach((m) => { wallet.set(m.id, { shopping: 0n, income: 0n }); volume.set(m.id, { self: 0, group: 0, rank: 0 }); });

  const uplineOf = (m: typeof members[number]) =>
    m.ancestorPath.split('/').filter(Boolean).reverse()
      .map((id) => members.find((x) => x.id === id))
      .filter((x): x is typeof members[number] => !!x);
  ...
  for (let mi = 0; mi < MONTHS; mi++) {
    ...
    for (const m of members) {
      ...
      if (row.joinedAt >= monthEnd) continue;
      if (row.status !== 'ACTIVE' && rnd() > 0.2) continue;
      ...
      const targetBv = isFirstMonth ? bvToCenti(2000 + int(0, 600))
        : seller ? bvToCenti(int(600, 3000))
        : rnd() < 0.7 ? bvToCenti(int(500, 560))
        : rnd() < 0.5 ? 0 : bvToCenti(int(200, 450));
      if (targetBv <= 0) continue;

      const when = new Date(monthStart.getTime() + int(1, 24) * 864e5);
      if (when > now) continue;

      // --- build the basket ---
      const lines: { product: typeof products[number]; qty: number }[] = [];
      ...
      const status = delivered ? 'DELIVERED' as const : pick(['PLACED', 'PACKED', 'SHIPPED'] as const);
      ...
      await prisma.monthlyVolume.upsert({
        where: { memberId_period: { memberId: m.id, period: isoPeriod(deliveredAt!) } },
        create: { memberId: m.id, period: isoPeriod(deliveredAt!), selfBvCenti: bv, groupBvCenti: bv },
        update: { selfBvCenti: { increment: bv }, groupBvCenti: { increment: bv } },
      });

      const pay = async (target: typeof members[number], pctBp: number, type: 'SELF' | 'DIRECT' | 'TEAM' | 'GENERATION', gen?: number, depth?: number) => {
        const { amountPaise, remainderPaise } = commissionOn(bv, pctBp);
        ...
        await postLedger(target.id, 'INCOME', 'CREDIT', amountPaise, ledgerCategory as never, w.income, deliveredAt!, dedupeKey, order.id);
        commissionCount++;
      };

      const selfBp = rankAt(plan, volume.get(m.id)!.rank).selfPctBp;
      await pay(m, selfBp, 'SELF');
      if (isFirstMonth && chain[0]) await pay(chain[0], plan.direct.pctBp, 'DIRECT', undefined, 1);

      let paidBp = selfBp;
      const topBp = Math.max(...plan.ranks.map((r) => r.selfPctBp));
      for (const [i, u] of chain.entries()) {
        if (paidBp >= topBp) break;
        const upBp = rankAt(plan, volume.get(u.id)!.rank).selfPctBp;
        if (upBp > paidBp) { await pay(u, upBp - paidBp, 'TEAM', undefined, i + 1); paidBp = upBp; }
      }
      let gen = 0;
      for (const [i, u] of chain.entries()) {
        if (gen >= plan.generation.levelsBp.length) break;
        if (volume.get(u.id)!.rank < plan.generation.minRankIndex) continue;
      ...
      for (const target of [m, ...chain]) {
        ...
        const next = rankIndexFor(plan, basis);
        if (next > v.rank) {
  ...
  for (const [i, m] of busy.entries()) {
    await prisma.recharge.create({
      data: {
        memberId: m.id, claimedPaise: rupeesToPaise([5000, 2000, 30000][i]),
        utr: String(100000000000 + Math.floor(rnd() * 899999999999)),
  ...
  if (earner && earner[1].income > rupeesToPaise(600)) {
    const amount = rupeesToPaise(500);
    const deduction = (amount * BigInt(CLIENT_DEFAULT_PLAN.withdrawal.deductionBp)) / 10000n;
    await prisma.withdrawal.create({
      data: {
        memberId: earner[0], requestedPaise: amount, deductionPaise: deduction, netPaise: amount - deduction,
        deductionBp: CLIENT_DEFAULT_PLAN.withdrawal.deductionBp, status: 'PENDING',
        payoutSnapshot: { upi: 'demo@ybl' }, createdAt: new Date(now.getTime() - 6 * 36e5),
      },
    });
  }

  // The orders above get their numbers from a plain counter (`orderNo`,
  // `invoiceNo`) rather than `OrderService.nextNumber()`, so `NumberSeries`
  // itself never advances past its bootstrap value. Left alone, the first
  // real checkout after seeding collides on the very first number it tries —
  // this is what actually keeps that from happening every time the demo data
  // is reset.
  const maxOrderNo = await prisma.$queryRaw<{ max: number | null }[]>`
    SELECT MAX(CAST(SUBSTRING("orderNo" FROM 3) AS INTEGER)) AS max FROM "Order" WHERE "orderNo" LIKE 'OD%'
  `;
  if (maxOrderNo[0]?.max) {
    await prisma.$executeRaw`
      INSERT INTO "NumberSeries" (key, prefix, "nextValue", "updatedAt")
      VALUES ('order', 'OD', ${maxOrderNo[0].max + 2}, NOW())
      ON CONFLICT (key) DO UPDATE SET "nextValue" = GREATEST("NumberSeries"."nextValue", ${maxOrderNo[0].max + 2}), "updatedAt" = NOW()
    `;
  }
  const invoiceYears = await prisma.$queryRaw<{ fy: string; max: number }[]>`
    SELECT SUBSTRING("invoiceNo" FROM 5 FOR 7) AS fy, MAX(CAST(SUBSTRING("invoiceNo" FROM 13) AS INTEGER)) AS max
    FROM "Order" WHERE "invoiceNo" IS NOT NULL GROUP BY fy
  `;
  for (const { fy, max } of invoiceYears) {
    await prisma.$executeRaw`
      INSERT INTO "NumberSeries" (key, prefix, "nextValue", "updatedAt")
      VALUES (${`invoice:${fy}`}, ${`INV/${fy}/`}, ${max + 2}, NOW())
      ON CONFLICT (key) DO UPDATE SET "nextValue" = GREATEST("NumberSeries"."nextValue", ${max + 2}), "updatedAt" = NOW()
    `;
  }
  ...
  console.log(`  revenue ${formatInr(totalRevenue._sum.totalPaise ?? 0n)}, commission ${formatInr(totalCommission._sum.amountPaise ?? 0n)}`);
  console.log('\n  Demo member login: 9876510000 / demo123456');
  console.log(`  Demo admin login:  ${ctx.adminEmail} / ${ctx.adminPassword}\n`);
}

let journalCounter = 0;
async function postLedger(
  memberId: string, kind: 'SHOPPING' | 'INCOME', direction: 'CREDIT' | 'DEBIT',
  amountPaise: bigint, category: string, balanceAfter: bigint, at: Date, idempotencyKey: string, refId?: string,
) {
  ...
  await prisma.ledgerEntry.create({
    data: {
      ...
      amountPaise, category: category as never, balanceAfter,
